...
function generateSuggestions(analysis) {
  ...
  if (!analysis.hasReadme) {
    s.push({
      id: 'missing-readme', priority: 'high', category: 'Documentation',
      icon: '📋', title: 'Add a README file',
  ...
  if (!analysis.hasLicense) {
    s.push({
      id: 'missing-license', priority: 'high', category: 'Legal',
      icon: '⚖️', title: 'Add an open source license',
  ...
  if (!analysis.hasContributing) {
    s.push({
      id: 'missing-contributing', priority: 'medium', category: 'Documentation',
      icon: '🤝', title: 'Add a CONTRIBUTING guide',
  ...
  if (!analysis.hasGitignore) {
    s.push({
      id: 'missing-gitignore', priority: 'high', category: 'Security',
      icon: '🔒', title: 'Add a .gitignore file',
  ...
  if (!analysis.hasEnvFile && analysis.envVars.length > 0) {
    s.push({
      id: 'missing-env-example', priority: 'high', category: 'Security',
      icon: '🔑', title: 'Create a .env.example file',
  ...
  if (analysis.envVars.some(v => v.toLowerCase().includes('secret') || v.toLowerCase().includes('key') || v.toLowerCase().includes('password'))) {
    s.push({
      id: 'sensitive-env-vars', priority: 'medium', category: 'Security',
      icon: '🛡️', title: 'Document sensitive environment variables',
  ...
  if (!analysis.hasTests) {
    s.push({
      id: 'missing-tests', priority: 'high', category: 'Testing',
      icon: '🧪', title: 'Add automated tests',
    ...
    s.push({
      id: 'tests-no-ci', priority: 'medium', category: 'CI/CD',
      icon: '⚙️', title: 'Set up a CI/CD pipeline',
  ...
  if (!analysis.hasDocker) {
    s.push({
      id: 'add-docker', priority: 'low', category: 'DevOps',
      icon: '🐳', title: 'Add Docker support',
  ...
  if (analysis.hasReadme && !analysis.description) {
    s.push({
      id: 'readme-no-description', priority: 'medium', category: 'Documentation',
      icon: '✍️', title: 'Add a project description',
  ...
  if (depCount > 50) {
    s.push({
      id: 'many-deps', priority: 'medium', category: 'Performance',
      icon: '📦', title: 'Review your dependency tree',
  ...
  if (analysis.apiRoutes.length > 0 && !analysis.authentication) {
    s.push({
      id: 'api-no-auth', priority: 'high', category: 'Security',
      icon: '🔐', title: 'Add authentication to your API',
  ...
  if (analysis.apiRoutes.length > 0) {
    s.push({
      id: 'add-api-versioning', priority: 'low', category: 'Architecture',
      icon: '🔢', title: 'Consider API versioning',
  ...
  if (!analysis.buildTool && analysis.primaryLanguage === 'JavaScript') {
    s.push({
      id: 'add-build-tool', priority: 'low', category: 'Developer Experience',
      icon: '⚡', title: 'Add a build tool',
  ...
  if (analysis.primaryLanguage === 'JavaScript' && !analysis.devDependencies['typescript'] && !analysis.dependencies['typescript']) {
    s.push({
      id: 'consider-typescript', priority: 'low', category: 'Code Quality',
      icon: '🔷', title: 'Consider migrating to TypeScript',
  ...
  // ── Performance ──
  if (analysis.database && analysis.database !== 'Redis' && !Object.keys(analysis.dependencies).some(d => d.includes('redis') || d.includes('cache'))) {
    s.push({
      id: 'add-caching', priority: 'low', category: 'Performance',
      icon: '⚡', title: 'Add a caching layer',
  ...
  if (analysis.envVars.length > 0 && !analysis.hasEnvFile) {
    s.push({
      id: 'document-env-vars', priority: 'medium', category: 'Documentation',
      icon: '📝', title: 'Document all environment variables',
