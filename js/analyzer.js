...
const JAVA_ENTITY_SUFFIXES = [
  'Controller','RestController','Service','ServiceImpl','Repository','RepositoryImpl',
  'DAO','DAOImpl','DAOImpl','Manager','ManagerImpl','Handler','Processor','Listener',
  'EventListener','Factory','Builder','Validator','Converter','Mapper','Adapter',
  'Provider','Client','Consumer','Producer','Worker','Runner','Helper','Util','Utils',
  'Impl','Event','Request','Response','DTO','Bean','Config','Configuration','Exception',
  'Facade','Proxy','Observer','Strategy','Decorator','Command','Template','Visitor',
  'Mediator','Iterator','Singleton','State','Chain',
];

const SKIP_ENTITY_NAMES = new Set([
  'Main','App','Application','Config','Security','Exception','Error','Base','Abstract',
  'Generic','Common','Util','Utils','Helper','Constants','Test','Tests','Spec','Mock',
  'Stub','Fake','Dummy','Index','Home','Default','Root','Global',
]);
...
const LANG_COLORS = {
  ...
  Java:'#b07219',Kotlin:'#A97BFF',Scala:'#c22d40',
  Go:'#00ADD8',Rust:'#dea584',Ruby:'#701516',
  PHP:'#4F5D95','C#':'#178600','C++':'#f34b7d',C:'#555555',
  Swift:'#fa7343',Dart:'#00B4AB',Vue:'#41b883',
...
const IGNORE_DIRS = new Set([
  'node_modules','.git','__pycache__','target','build','out','dist','.idea',
  '.vscode','coverage','.cache','bin','obj','.gradle','.mvn','venv','env',
  '.next','.nuxt','vendor','.DS_Store','__MACOSX','logs','tmp','temp',
]);
...
function computeLanguageStats(files) {
  ...
  const stats  = {};
  for (const [l, c] of Object.entries(counts)) stats[l] = Math.max(1, Math.round(c / total * 100));
...
function analyzeJavaFile(content, filepath) {
  ...
  if (entity.className) {
    const extRx  = new RegExp(`class\\s+${entity.className}[\\w<>,\\s]*\\s+extends\\s+([\\w<>,\\s.]+?)(?:\\s+implements|\\s*\\{)`);
    const implRx = new RegExp(`class\\s+${entity.className}[\\w<>,\\s]*(?:\\s+extends[\\w<>,\\s.]+)?\\s+implements\\s+([\\w<>,\\s.]+?)\\s*\\{`);
    const extM   = content.match(extRx);
    const implM  = content.match(implRx);
    if (extM)  entity.extendsClass         = extM[1].trim().split(/[<\s]/)[0];
    if (implM) entity.implementsInterfaces = implM[1].split(',').map(s => s.trim().split(/[<\s]/)[0]);
  }
  ...
  const methodRx = /(?:public|protected)\s+(?:static\s+)?(?:(?:final|synchronized|abstract|native|default)\s+)*(?:[\w<>\[\]?,\s]+?)\s+(\w+)\s*\([^)]*\)\s*(?:throws\s+[\w,\s]+)?\s*[{;]/gm;
  const BAD_NAMES = new Set(['if','while','for','switch','try','catch','return','class','interface','new','void','else']);
  entity.methods = [...new Set([...content.matchAll(methodRx)].map(m => m[1]).filter(n => n && !BAD_NAMES.has(n)))];

  // Route mappings (@GetMapping, @PostMapping, etc.)
  const routeRx = /@(GetMapping|PostMapping|PutMapping|DeleteMapping|PatchMapping|RequestMapping)\s*(?:\(\s*(?:value\s*=\s*)?(?:\{?\s*["']([^"']*)["']\}?)?)?/g;
  entity.routeMappings = [...content.matchAll(routeRx)].map(m => ({ annotation: m[1], path: m[2] || '' }));
  ...
  // Field declarations
  const fieldRx = /(?:private|protected|public)\s+(?:static\s+)?(?:final\s+)?(?:[\w<>\[\]?]+)\s+(\w+)\s*[=;]/gm;
  entity.fields = [...new Set([...content.matchAll(fieldRx)].map(m => m[1]).filter(n => n !== 'serialVersionUID'))];
...
function analyzePythonFile(content, filepath) {
  ...
  const flaskRx = /@(?:app|router|blueprint|bp|api)\.(?:route|get|post|put|delete|patch)\s*\(\s*["']([^"']+)["']/gm;
  ...
  entity.isEntryPoint = content.includes("if __name__ == '__main__'") || content.includes('if __name__ == "__main__"');
...
function analyzeJSFile(content, filepath) {
  ...
  const compRx = /(?:export\s+(?:default\s+)?)?(?:function|const)\s+([A-Z][A-Za-z0-9_]*)\s*(?:=\s*(?:\([^)]*\)|[\w]+)\s*=>|\()/gm;
  entity.reactComponents = [...content.matchAll(compRx)].map(m => m[1])
    .filter(n => !['Promise','Array','Object','String','Number','Boolean'].includes(n));

  // Custom hooks
  entity.hooks = [...content.matchAll(/(?:const|function)\s+(use[A-Z]\w*)\s*(?:=|\()/gm)].map(m => m[1]);

  // Express routes
  const routeRx = /(?:router|app)\.(get|post|put|delete|patch|head|all)\s*\(\s*['"`]([^'"`]+)['"`]/gm;
  entity.routes = [...content.matchAll(routeRx)].map(m => ({ method: m[1].toUpperCase(), path: m[2] }));

  entity.isEntryPoint = /\.(listen|createServer)\s*\(/.test(content) ||
...
function parsePomXml(content) {
  ...
  const r = {
    ...
    name: null, description: null, packaging: null,
    javaVersion: null, springBootVersion: null, parentArtifact: null,
  ...
  // Java version
  const jvM = content.match(/<java\.version>([^<]+)<\/java\.version>/) ||
  ...
  r.dependencies = depBlocks.map(m => ({
...
function parseBuildGradle(content) {
  if (!content) return null;
  const r = { group: null, version: null, javaVersion: null, springBootVersion: null, dependencies: [], plugins: [] };
  ...
  const depRx = /(?:implementation|compile|api|testImplementation|runtimeOnly|compileOnly|annotationProcessor)\s*[\("']([^)"'\n]+)["')]/g;
...
function parseApplicationProperties(content) {
  ...
  return {
    ...
    jpaDialect:       props['spring.jpa.database-platform'] || props['spring.jpa.properties.hibernate.dialect'] || null,
    activeProfile:    props['spring.profiles.active'] || null,
    securityEnabled:  'spring.security.user.password' in props || 'spring.security.user.name' in props,
    raw: props,
  };
}
...
function parseCargoToml(content) {
  if (!content) return null;
  const r = { name: null, version: null, description: null, edition: null, authors: [], dependencies: [] };
...
function parseGoMod(content) {
  ...
  const reqBlock = content.match(/require\s*\(([^)]+)\)/s) || content.match(/require\s*\(([^)]+)\)/m);
  if (reqBlock) {
...
function parsePyprojectToml(content) {
  if (!content) return null;
  const r = { name: null, version: null, description: null, authors: [], dependencies: [], buildBackend: null };
  ...
  const depSection = content.match(/\[tool\.poetry\.dependencies\]([\s\S]*?)(?=^\[|\Z)/m) ||
                     content.match(/\[project\][\s\S]*?dependencies\s*=\s*\[([^\]]*)\]/m);
  if (depSection) {
    r.dependencies = [...depSection[1].matchAll(/^\s*([\w-]+)\s*[=^~<>]/gm)].map(m => m[1])
      .filter(n => n !== 'python');
  }
  return r;
}
...
function analyzeSqlFile(content, filepath) {
  if (!content) return null;
  const tables    = [...content.matchAll(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"']?(\w+)[`"']?\s*\(/gi)].map(m => m[1]);
  const procs     = [...content.matchAll(/CREATE\s+(?:OR\s+REPLACE\s+)?(?:PROCEDURE|FUNCTION)\s+[`"']?(\w+)/gi)].map(m => m[1]);
  const views     = [...content.matchAll(/CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+[`"']?(\w+)/gi)].map(m => m[1]);
...
function detectFramework(javaClasses, pythonEntities, jsModules, pomData, gradleData, pkgJson, reqPackages) {
  const out = {
    primary: null, primaryConfidence: 0, primaryEvidence: [],
    ui: null, uiConfidence: 0, uiEvidence: [],
    data: null, dataConfidence: 0, dataEvidence: [],
  };
  ...
  if (javaClasses.length > 0) {
    ...
    const pomDeps        = (pomData?.dependencies || []).map(d => `${d.groupId || ''}:${d.artifactId || ''}`).join(' ').toLowerCase();
    ...
    const hasSBDep        = allDeps.includes('spring-boot-starter') || gradleData?.plugins?.some(p => p.includes('spring-boot'));
    ...
    if (hasSBAnnotation || hasSBParent || hasSBDep || hasSBPlugin) {
      ...
      if (hasSBDep)     ev.push(`spring-boot-starter-* dependency in ${pomData ? 'pom.xml' : 'build.gradle'}`);
      if (hasSBPlugin && !hasSBParent && !hasSBDep) ev.push('Spring Boot Maven/Gradle plugin detected');
    ...
    if (!out.primary) {
      const hasMVC = allDeps.includes('spring-webmvc') || allDeps.includes('spring-web') || allDeps.includes('spring-framework');
      const hasCtrl = allAnnotations.some(a => ['Controller','RestController'].includes(a));
      if (hasMVC && hasCtrl) {
        out.primary = 'Spring MVC'; out.primaryConfidence = 88;
        out.primaryEvidence = ['spring-webmvc dependency detected', '@Controller or @RestController annotation found'];
      }
    }

    // JavaFX
    const javafxImports = javaClasses.flatMap(e => e.imports.filter(i => i.startsWith('javafx.')));
    const javafxDep     = allDeps.includes('javafx') || (pomData?.dependencies || []).some(d => (d.groupId || '').includes('javafx'));
    const hasFXML       = javaClasses.some(e => e.imports.some(i => i.includes('javafx.fxml')));
    if (javafxImports.length > 0 || javafxDep) {
      out.ui = 'JavaFX'; out.uiConfidence = Math.min(100, 70 + javafxImports.length * 5);
      ...
      if (!out.primary) { out.primary = 'JavaFX'; out.primaryConfidence = out.uiConfidence; out.primaryEvidence = out.uiEvidence; }
    }

    // Java Swing
    if (!out.ui) {
      const swingImports = javaClasses.flatMap(e => e.imports.filter(i => i.startsWith('javax.swing') || (i.startsWith('java.awt') && !i.startsWith('java.awt.geom'))));
      if (swingImports.length > 0) {
        out.ui = 'Java Swing'; out.uiConfidence = Math.min(100, 65 + swingImports.length * 5);
        out.uiEvidence = [...new Set(swingImports.slice(0,3).map(i => `import ${i}`))];
        if (!out.primary) { out.primary = 'Java Swing'; out.primaryConfidence = out.uiConfidence; out.primaryEvidence = out.uiEvidence; }
      }
    }

    // Android
    if (!out.primary && (allImports.includes('android.app') || allImports.includes('androidx') || allDeps.includes('com.android'))) {
      out.primary = 'Android'; out.primaryConfidence = 92;
      out.primaryEvidence = ['android.* imports detected'];
    }

    // Core Java fallback
    if (!out.primary) {
      out.primary = 'Core Java'; out.primaryConfidence = 85;
      out.primaryEvidence = ['No Spring Boot, JavaFX, Swing, or Android indicators found'];
    }

    // ORM / Data Access
    const hasJpaAnnot  = allAnnotations.some(a => ['Entity','Table','Document','MappedSuperclass'].includes(a));
    const hasJpaImport = allImports.includes('javax.persistence') || allImports.includes('jakarta.persistence') || allDeps.includes('spring-data-jpa') || allDeps.includes('hibernate');
    const hasJdbc      = allImports.includes('java.sql.Connection') || allImports.includes('java.sql.DriverManager') || allImports.includes('java.sql.PreparedStatement');
    const hasMyBatis   = allDeps.includes('mybatis') || allImports.includes('org.mybatis');

    if (hasJpaAnnot || hasJpaImport) {
      out.data = 'JPA / Hibernate'; out.dataConfidence = hasJpaAnnot && hasJpaImport ? 100 : 85;
      out.dataEvidence = [];
      if (hasJpaAnnot) {
        const ef = javaClasses.find(e => e.annotations.includes('Entity'));
        if (ef) out.dataEvidence.push(`@Entity annotation in ${ef.filename}`);
      }
      if (allImports.includes('javax.persistence'))  out.dataEvidence.push('import javax.persistence.*');
      if (allImports.includes('jakarta.persistence')) out.dataEvidence.push('import jakarta.persistence.*');
      if (allDeps.includes('spring-data-jpa'))        out.dataEvidence.push('spring-boot-starter-data-jpa in pom.xml');
    } else if (hasMyBatis) {
      out.data = 'MyBatis'; out.dataConfidence = 88; out.dataEvidence = ['mybatis dependency detected'];
    } else if (hasJdbc) {
      out.data = 'JDBC'; out.dataConfidence = 85; out.dataEvidence = ['java.sql imports detected (direct JDBC access)'];
    }
  }

  /* ── Python ── */
  if (pythonEntities.length > 0 && !out.primary) {
    const allI  = pythonEntities.flatMap(e => [...e.directImports, ...e.fromImports]).map(s => s.toLowerCase());
    const reqN  = reqPackages.map(p => p.name.toLowerCase());
    const has   = (n) => allI.some(i => i.startsWith(n)) || reqN.some(r => r.startsWith(n));

    let pyFw = null;
    if (has('django'))     { pyFw = 'Django';    out.primaryConfidence = 95; }
    else if (has('fastapi'))  { pyFw = 'FastAPI';   out.primaryConfidence = 95; }
    else if (has('flask'))    { pyFw = 'Flask';     out.primaryConfidence = 95; }
    else if (has('tornado'))  { pyFw = 'Tornado';   out.primaryConfidence = 90; }
    else if (has('streamlit')){ pyFw = 'Streamlit'; out.primaryConfidence = 90; }
    else if (has('pygame'))   { pyFw = 'Pygame';    out.primaryConfidence = 92; }
    else if (has('tkinter') || allI.some(i => i === 'tkinter')) {
      pyFw = 'Python (Tkinter GUI)'; out.primaryConfidence = 92; out.ui = 'Tkinter'; out.uiConfidence = 95;
    }

    if (pyFw) {
      out.primary = pyFw;
      const ev = [];
      if (allI.some(i => i.startsWith(pyFw.toLowerCase().split(' ')[0]))) ev.push(`${pyFw.toLowerCase().split(' ')[0]} import in source code`);
      if (reqN.some(r => r.startsWith(pyFw.toLowerCase().split(' ')[0]))) ev.push(`${pyFw} in requirements.txt`);
      out.primaryEvidence = ev;
    }

    // Python ORM
    if (has('sqlalchemy')) { out.data = 'SQLAlchemy'; out.dataConfidence = 95; out.dataEvidence = ['sqlalchemy import detected']; }
    else if (has('pymongo') || has('mongoengine')) { out.data = 'PyMongo/MongoEngine'; out.dataConfidence = 92; out.dataEvidence = ['pymongo import detected']; }
    else if (has('psycopg') || has('psycopg2'))   { out.data = 'psycopg2 (PostgreSQL)'; out.dataConfidence = 92; out.dataEvidence = ['psycopg2 import detected']; }
    else if (has('peewee'))  { out.data = 'Peewee ORM'; out.dataConfidence = 90; out.dataEvidence = ['peewee import detected']; }
  }
  ...
  if (jsModules.length > 0 && !out.primary && pkgJson) {
    ...
    if (hasDep('next'))                     { out.primary = 'Next.js';    out.primaryConfidence = 97; }
    else if (hasDep('nuxt') || hasDep('nuxt3'))  { out.primary = 'Nuxt.js';    out.primaryConfidence = 97; }
    else if (hasDep('@sveltejs/kit'))        { out.primary = 'SvelteKit';  out.primaryConfidence = 97; }
    else if (hasDep('gatsby'))               { out.primary = 'Gatsby';     out.primaryConfidence = 97; }
    else if (hasDep('@remix-run/react'))     { out.primary = 'Remix';      out.primaryConfidence = 97; }
    else if (hasDep('astro'))                { out.primary = 'Astro';      out.primaryConfidence = 97; }
    // UI frameworks
    else if (hasDep('react') || hasDep('react-dom')) { out.ui = 'React'; out.uiConfidence = 97; out.uiEvidence = ['react in package.json']; }
    else if (hasDep('vue'))                  { out.ui = 'Vue.js'; out.uiConfidence = 97; out.uiEvidence = ['vue in package.json']; }
    else if (hasDep('@angular/core'))        { out.ui = 'Angular'; out.uiConfidence = 97; out.uiEvidence = ['@angular/core in package.json']; }
    else if (hasDep('svelte'))               { out.ui = 'Svelte'; out.uiConfidence = 97; out.uiEvidence = ['svelte in package.json']; }

    // Backend frameworks
    if (!out.primary) {
      if (hasDep('@nestjs/core'))   { out.primary = 'NestJS';     out.primaryConfidence = 97; }
      else if (hasDep('express'))   { out.primary = 'Express.js'; out.primaryConfidence = 95; }
      else if (hasDep('fastify'))   { out.primary = 'Fastify';    out.primaryConfidence = 95; }
      else if (hasDep('koa'))       { out.primary = 'Koa.js';     out.primaryConfidence = 95; }
      else if (hasDep('hono'))      { out.primary = 'Hono';       out.primaryConfidence = 95; }
      else if (out.ui)              { out.primary = out.ui;        out.primaryConfidence = out.uiConfidence; }
    }
    if (out.primary) {
    ...
    // JS ORM
    if (!out.data) {
      if (hasDep('@prisma/client') || hasDep('prisma')) { out.data = 'Prisma'; out.dataConfidence = 97; out.dataEvidence = ['@prisma/client in package.json']; }
      else if (hasDep('mongoose'))  { out.data = 'Mongoose'; out.dataConfidence = 97; out.dataEvidence = ['mongoose in package.json']; }
      else if (hasDep('sequelize')) { out.data = 'Sequelize'; out.dataConfidence = 97; out.dataEvidence = ['sequelize in package.json']; }
      else if (hasDep('typeorm'))   { out.data = 'TypeORM'; out.dataConfidence = 97; out.dataEvidence = ['typeorm in package.json']; }
      else if (hasDep('drizzle-orm')) { out.data = 'Drizzle ORM'; out.dataConfidence = 97; out.dataEvidence = ['drizzle-orm in package.json']; }
    }
  }
...
function detectDatabase(javaClasses, pythonEntities, jsModules, appProps, appYml, pomData, gradleData, pkgJson, reqPackages) {
  const r = { type: null, confidence: 0, evidence: [], orm: null, ormEvidence: [], ormConfidence: 0 };
  ...
  if (!r.type && pomData) {
    const artifactStr = (pomData.dependencies || []).map(d => (d.artifactId || '')).join(' ').toLowerCase();
    const groupStr    = (pomData.dependencies || []).map(d => (d.groupId || '')).join(' ').toLowerCase();
  ...
  if (!r.type && reqPackages.length > 0) {
    ...
    const REQ_DB_MAP = [
      ['psycopg',    'PostgreSQL'], ['pg8000',    'PostgreSQL'],
  ...
  // 5. package.json (90%)
  if (!r.type && pkgJson) {
    const deps = Object.keys({ ...(pkgJson.dependencies || {}), ...(pkgJson.devDependencies || {}) }).join(' ').toLowerCase();
    const PKG_DB_MAP = [
      ['mongoose',        'MongoDB'],
      ['mongodb',         'MongoDB'],
      ['pg',              'PostgreSQL'],
  ...
  if (!r.type && javaClasses.length > 0) {
    const allImports = javaClasses.flatMap(e => e.imports).join(' ').toLowerCase();
    const JDBC_MAP = [
      ['mysql',    'MySQL'], ['postgres', 'PostgreSQL'],
      ['oracle',   'Oracle DB'], ['sqlite', 'SQLite'],
      ['mariadb',  'MariaDB'],
    ];
    for (const [kw, name] of JDBC_MAP) {
      if (allImports.includes(kw)) {
        r.type = name; r.confidence = 70;
  ...
  if (javaClasses.length > 0) {
    ...
    const pomDepStr = (pomData?.dependencies || []).map(d => d.artifactId || '').join(' ').toLowerCase();

    if (anns.includes('Entity') || imps.includes('javax.persistence') || imps.includes('jakarta.persistence') || pomDepStr.includes('data-jpa') || pomDepStr.includes('hibernate')) {
      r.orm = 'JPA / Hibernate'; r.ormConfidence = anns.includes('Entity') ? 98 : 85;
      r.ormEvidence = [];
      if (anns.includes('Entity')) r.ormEvidence.push(`@Entity annotation found in ${javaClasses.find(e => e.annotations.includes('Entity'))?.filename}`);
      ...
      if (pomDepStr.includes('data-jpa'))       r.ormEvidence.push('spring-boot-starter-data-jpa in pom.xml');
    }
  }
  if (!r.orm && pkgJson) {
    const deps = Object.keys({ ...(pkgJson.dependencies || {}), ...(pkgJson.devDependencies || {}) });
    const ORM_MAP = [['@prisma/client','Prisma'],['mongoose','Mongoose'],['sequelize','Sequelize'],['typeorm','TypeORM'],['drizzle-orm','Drizzle ORM']];
    for (const [pkg, name] of ORM_MAP) {
      if (deps.includes(pkg)) { r.orm = name; r.ormConfidence = 97; r.ormEvidence = [`${pkg} in package.json`]; break; }
    }
  }
...
function detectAuthentication(javaClasses, pkgJson, pomData, gradleData, reqPackages) {
  ...
  const imps    = javaClasses.flatMap(e => e.imports).join(' ').toLowerCase();
  const pomDeps = (pomData?.dependencies || []).map(d => (d.artifactId || '')).join(' ').toLowerCase() +
                  ' ' + (pomData?.dependencies || []).map(d => (d.groupId || '')).join(' ').toLowerCase();
  const gradleDeps = (gradleData?.dependencies || []).join(' ').toLowerCase();
  const allJavaDeps = pomDeps + ' ' + gradleDeps;
  const pkgDeps = Object.keys({ ...(pkgJson?.dependencies || {}), ...(pkgJson?.devDependencies || {}) });
  const reqNames = reqPackages.map(p => p.name.toLowerCase());

  // Spring Security
  if (allJavaDeps.includes('spring-security') || anns.includes('EnableWebSecurity') || anns.includes('PreAuthorize') || anns.includes('Secured')) {
    const ev = [];
  ...
  // Java JWT
  if (allJavaDeps.includes('jjwt') || allJavaDeps.includes('java-jwt') || imps.includes('io.jsonwebtoken')) {
  ...
  for (const [pkg, name] of JS_AUTH_MAP) {
    if (pkgDeps.includes(pkg)) return { name, confidence: 95, evidence: [`${pkg} in package.json`] };
  }
  // Auth0 (namespaced)
  if (pkgDeps.some(d => d.startsWith('@auth0/'))) return { name: 'Auth0', confidence: 95, evidence: ['@auth0/* in package.json'] };
  // Python auth
  if (reqNames.some(r => r.includes('pyjwt') || r.includes('python-jose'))) return { name: 'JWT (PyJWT)', confidence: 88, evidence: ['PyJWT in requirements.txt'] };
  if (reqNames.some(r => r.includes('django-allauth')))   return { name: 'Django AllAuth', confidence: 90, evidence: ['django-allauth in requirements.txt'] };
  if (reqNames.some(r => r.includes('simplejwt')))        return { name: 'Django Simple JWT', confidence: 90, evidence: ['djangorestframework-simplejwt in requirements.txt'] };
  if (reqNames.some(r => r.includes('flask-login')))      return { name: 'Flask-Login', confidence: 90, evidence: ['Flask-Login in requirements.txt'] };
...
function detectTestFramework(javaClasses, pkgJson, pomData, gradleData, reqPackages) {
  ...
  const gradleDeps = (gradleData?.dependencies || []).join(' ').toLowerCase();
  const allJavaDeps = pomDeps + ' ' + gradleDeps;
  const pkgDeps = Object.keys({ ...(pkgJson?.devDependencies || {}), ...(pkgJson?.dependencies || {}) });
  const reqNames = reqPackages.map(p => p.name.toLowerCase());

  // Java
  if (allJavaDeps.includes('junit-jupiter') || allJavaDeps.includes('junit5')) return { name: 'JUnit 5', confidence: 97, evidence: ['junit-jupiter dependency'] };
  if (allJavaDeps.includes('testng')) return { name: 'TestNG', confidence: 97, evidence: ['testng dependency'] };
  if (allJavaDeps.includes('junit') && !allJavaDeps.includes('junit-jupiter')) {
    if (imps.includes('org.junit.jupiter')) return { name: 'JUnit 5', confidence: 92, evidence: ['@Test from JUnit 5 imports'] };
  ...
  if (anns.includes('Test')) {
    if (imps.includes('org.junit.jupiter')) return { name: 'JUnit 5', confidence: 88, evidence: ['@Test + JUnit 5 import'] };
    if (imps.includes('org.testng'))        return { name: 'TestNG', confidence: 88, evidence: ['@Test + TestNG import'] };
  ...
  // Python
  if (reqNames.some(r => r.includes('pytest')))   return { name: 'pytest', confidence: 95, evidence: ['pytest in requirements.txt'] };
  if (reqNames.some(r => r.includes('unittest'))) return { name: 'unittest', confidence: 90, evidence: ['unittest in requirements.txt'] };
...
function extractFeatures(javaClasses, pythonEntities, jsModules, sqlData) {
  ...
  for (const cls of javaClasses) {
    ...
    if (cls.annotations.some(a => ['RestController','Controller'].includes(a))) layerType = 'controller';
    if (cls.annotations.includes('Service'))    layerType = 'service';
    if (cls.annotations.includes('Repository')) layerType = 'repository';
    if (cls.annotations.some(a => ['Entity','Table','Document'].includes(a))) { layerType = 'entity'; entityName = cls.className; }

    if (SKIP_ENTITY_NAMES.has(entityName) || entityName.length < 2) continue;

    if (!map.has(entityName)) map.set(entityName, { name: entityName, classes: [], types: new Set(), evidence: [], methods: [] });
  ...
  for (const pyf of pythonEntities) {
    for (const cls of pyf.classes) {
      ...
      let entityName = cls;
      for (const suffix of ['View','ViewSet','APIView','Serializer','Form','Filter','Admin','Model','Manager','Schema']) {
        if (cls.endsWith(suffix) && cls.length > suffix.length + 1) { entityName = cls.slice(0, -suffix.length); break; }
      }
      if (SKIP_ENTITY_NAMES.has(entityName) || entityName.length < 2) continue;
      if (!map.has(entityName)) map.set(entityName, { name: entityName, classes: [], types: new Set(), evidence: [], methods: [] });
      const entry = map.get(entityName);
  ...
  for (const sqlF of sqlData) {
    for (const table of (sqlF.tables || [])) {
      ...
      const displayName = table.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()).replace(/s$/, '');
      if (SKIP_ENTITY_NAMES.has(displayName)) continue;
      if (!map.has(displayName)) map.set(displayName, { name: displayName, classes: [], types: new Set(), evidence: [], methods: [] });
...
function detectArchitecture(javaClasses) {
  const r = {
    pattern: null, patternConfidence: 0, patternEvidence: [],
    layers: { controllers: [], services: [], repositories: [], entities: [], configs: [], components: [] },
    packages: [],
    entryPoint: null, entryPointFile: null, entryPointConfidence: 0,
  };

  if (!javaClasses.length) return r;

  r.layers.controllers  = javaClasses.filter(e => e.annotations.some(a => ['RestController','Controller'].includes(a)) || e.className?.endsWith('Controller'));
  r.layers.services     = javaClasses.filter(e => e.annotations.includes('Service') || e.className?.endsWith('Service') || e.className?.endsWith('ServiceImpl'));
  r.layers.repositories = javaClasses.filter(e => e.annotations.includes('Repository') || e.className?.endsWith('Repository') || e.className?.endsWith('DAO') || e.className?.endsWith('DAOImpl'));
  r.layers.entities     = javaClasses.filter(e => e.annotations.some(a => ['Entity','Table','Document','MappedSuperclass'].includes(a)));
  r.layers.configs      = javaClasses.filter(e => e.annotations.includes('Configuration') || e.className?.endsWith('Config') || e.className?.endsWith('Configuration'));
  r.layers.components   = javaClasses.filter(e => e.annotations.includes('Component') && !r.layers.controllers.includes(e) && !r.layers.services.includes(e));
  ...
  if (C > 0 && S > 0 && R > 0) {
    ...
    r.patternEvidence = [
      `${C} Controller(s): ${r.layers.controllers.slice(0,3).map(c => c.filename).join(', ')}`,
      `${S} Service(s): ${r.layers.services.slice(0,3).map(s => s.filename).join(', ')}`,
      `${R} Repository/DAO(s): ${r.layers.repositories.slice(0,3).map(x => x.filename).join(', ')}`,
    ];
  } else if (C > 0 && S > 0) {
...
function detectDesignPatterns(javaClasses) {
  ...
  if (anns.includes('Scheduled'))      patterns.push({ name: 'Scheduler Pattern', evidence: ['@Scheduled annotation'] });
  if (anns.includes('EventListener'))  patterns.push({ name: 'Event-Driven Pattern', evidence: ['@EventListener annotation'] });
  if (anns.includes('Async'))          patterns.push({ name: 'Asynchronous Processing', evidence: ['@Async annotation'] });
  if (anns.includes('Cacheable'))      patterns.push({ name: 'Cache-Aside Pattern', evidence: ['@Cacheable annotation'] });
  if (anns.includes('Transactional'))  patterns.push({ name: 'Unit of Work (Transactional)', evidence: ['@Transactional annotation'] });
...
function collectApiRoutes(javaClasses, pythonEntities, jsModules) {
  ...
  for (const pyf of pythonEntities) {
    for (const route of pyf.routes) {
      routes.push({ method: 'HTTP', path: route, file: pyf.filepath, handler: pyf.filename, evidence: `Route in ${pyf.filename}` });
    }
  }
  ...
  for (const jsm of jsModules) {
    for (const route of jsm.routes) {
      routes.push({ method: route.method, path: route.path, file: jsm.filepath, handler: jsm.filename, evidence: `${route.method} route in ${jsm.filename}` });
    }
  }
...
function buildFolderTree(files) {
  ...
  for (const file of files) {
    const parts = (file.path || '').replace(/\\/g, '/').replace(/^\//, '').split('/').filter(Boolean);
    ...
    if (fname && !cur.children.find(c => c.name === fname && c.type === 'file')) {
      cur.children.push({ name: fname, type: 'file', size: file.size || 0, ext: getExt(fname), path: file.path, lineCount: file.content ? countLines(file.content) : null });
    }
  }
  return root;
}
...
function scanSecurityIssues(files) {
  ...
  const IGNORE_VALUES = new Set(['your_password_here','changeme','yourpassword','secret','password','example','placeholder','xxxxxxxxxx','your_secret_here']);

  for (const f of files.filter(f => f.content && !f.name.includes('.example') && !f.name.includes('.sample'))) {
    for (const { rx, label } of HARDCODED_PATTERNS) {
      ...
      while ((m = localRx.exec(f.content)) !== null) {
        ...
        if (!IGNORE_VALUES.has(val) && val !== '' && !/\$\{/.test(m[0]) && !/process\.env/.test(f.content.slice(Math.max(0, m.index-20), m.index+40))) {
...
function analyzeProject(files) {
  ...
  const jsFiles     = filtered.filter(f => ['.js','.jsx','.ts','.tsx','.mjs','.cjs','.cts','.mts'].includes(getExt(f.name)) && f.content);
  ...
  const gradleFile     = filtered.find(f => f.name === 'build.gradle' || f.name === 'build.gradle.kts');
  const pkgFile        = filtered.find(f => f.name === 'package.json' && !f.path.includes('node_modules'));
  const reqFile        = filtered.find(f => f.name === 'requirements.txt');
  const appPropFile    = filtered.find(f => f.name === 'application.properties');
  const appYmlFile     = filtered.find(f => f.name === 'application.yml' || f.name === 'application.yaml');
  const envFile        = filtered.find(f => ['.env','.env.example','.env.sample','.env.template','.env.local'].includes(f.name));
  // New config files
  const cargoFile      = filtered.find(f => f.name === 'Cargo.toml' || (f.name === 'Cargo.toml' && f.content));
  ...
  const readmeFile     = filtered.find(f => ['readme.md', 'readme.rst', 'readme.txt', 'readme'].includes(f.name.toLowerCase()) && f.content);

  const pomData        = parsePomXml(pomFile?.content);
  const gradleData     = parseBuildGradle(gradleFile?.content);
  const pkgJson        = pkgFile ? (() => { try { return JSON.parse(pkgFile.content); } catch { return null; } })() : null;
  ...
  if (readmeFile?.content) {
    ...
    for (const line of lines) {
      const t = line.trim();
      if (!t) { inContent = false; continue; }
      // Skip headings, badges, html tags, horizontal rules, toc
      if (t.startsWith('#')) { inContent = false; continue; }
  ...
  for (const f of [...javaFiles, ...pythonFiles, ...jsFiles, ...goFiles, ...rustFiles].slice(0, 50)) {
    if (!f.content) continue;
    for (const m of f.content.matchAll(/process\.env\.([A-Z_][A-Z0-9_]*)/g))   envSet.add(m[1]);
    for (const m of f.content.matchAll(/System\.getenv\s*\(\s*["']([^"']+)["']\s*\)/g)) envSet.add(m[1]);
    for (const m of f.content.matchAll(/os\.environ(?:\.get)?\s*\(\s*["']([^"']+)["']/g)) envSet.add(m[1]);
  ...
  const javaClasses    = javaFiles.slice(0,120).map(f => analyzeJavaFile(f.content, f.path)).filter(Boolean);
  const pythonEntities = pythonFiles.slice(0,80).map(f => analyzePythonFile(f.content, f.path)).filter(Boolean);
  const jsModules      = jsFiles.slice(0,80).map(f => analyzeJSFile(f.content, f.path)).filter(Boolean);
  const sqlData        = sqlFiles.map(f => analyzeSqlFile(f.content, f.path)).filter(Boolean);

  /* ── 6. Detections ── */
  const detectedFramework = detectFramework(javaClasses, pythonEntities, jsModules, pomData, gradleData, pkgJson, reqPkgs);
  const detectedDatabase  = detectDatabase(javaClasses, pythonEntities, jsModules, appProps, appYml, pomData, gradleData, pkgJson, reqPkgs);
  ...
  if (!detectedFramework.primary && goModData?.dependencies?.length > 0) {
    ...
    if (!detectedFramework.primary) {
      const allGoImports = goFiles.flatMap(f => [...(f.content?.matchAll(/import\s+"([^"]+)"/g) || [])].map(m => m[1])).join(' ');
      for (const [kw, name] of GO_FW_MAP) {
  ...
  const projectName = pkgJson?.name
    || pomData?.artifactId || pomData?.name
    || cargoData?.name
    || (goModData?.moduleName ? goModData.moduleName.split('/').pop() : null)
    || pyprojectData?.name
    || (() => {
      for (const f of filtered) {
        const parts = (f.path || '').replace(/\\/g,'/').split('/').filter(p => p && !IGNORE_DIRS.has(p));
  ...
  // Version
  const version = pkgJson?.version || pomData?.version || gradleData?.version
    || cargoData?.version || pyprojectData?.version || '';
  ...
  const hasDocker      = fnames.has('dockerfile') || fnames.has('docker-compose.yml') || fnames.has('docker-compose.yaml');
  const hasCI          = fpaths.some(p => p.includes('.github/workflows')) || fnames.has('.travis.yml') || fnames.has('.gitlab-ci.yml') || fnames.has('jenkinsfile');
  const hasTests       = detectedTest.name !== null ||
    filtered.some(f => f.name.includes('.test.') || f.name.includes('.spec.') ||
      f.name.endsWith('Test.java') || f.name.endsWith('Tests.java') ||
      (f.name.startsWith('Test') && f.name.endsWith('.java')) ||
      fpaths.some(p => p.includes('/test/') || p.includes('/tests/') || p.includes('/__tests__/')));

  /* ── 10. Build tool / Package manager ── */
  let packageManager = null, buildTool = null;
  if (pomData)         { packageManager = 'maven';    buildTool = 'Maven'; }
  else if (gradleData) { packageManager = 'gradle';   buildTool = 'Gradle'; }
  else if (fnames.has('pnpm-lock.yaml'))  { packageManager = 'pnpm';   buildTool = 'pnpm'; }
  else if (fnames.has('yarn.lock'))       { packageManager = 'yarn';   buildTool = 'Yarn'; }
  else if (fnames.has('bun.lockb'))       { packageManager = 'bun';    buildTool = 'Bun'; }
  else if (pkgJson)    { packageManager = 'npm';     buildTool = 'npm'; }
  else if (pyprojectFile) {
    if (fnames.has('poetry.lock') || pyprojectData?.buildBackend?.includes('poetry')) { packageManager = 'poetry'; buildTool = 'Poetry'; }
    else                                  { packageManager = 'pip';     buildTool = 'pip'; }
  }
  else if (reqFile) {
    if (fnames.has('poetry.lock'))        { packageManager = 'poetry';  buildTool = 'Poetry'; }
    else if (fnames.has('pipfile'))       { packageManager = 'pipenv';  buildTool = 'Pipenv'; }
    else                                  { packageManager = 'pip';     buildTool = 'pip'; }
  }
  else if (cargoFile || fnames.has('cargo.toml'))  { packageManager = 'cargo';   buildTool = 'Cargo'; }
  else if (goModFile  || fnames.has('go.mod'))      { packageManager = 'go mod';  buildTool = 'Go Modules'; }
  else if (fnames.has('composer.json'))             { packageManager = 'composer';buildTool = 'Composer'; }
  else if (fnames.has('gemfile'))                   { packageManager = 'bundler'; buildTool = 'Bundler'; }
  ...
  if (buildTool === 'npm' || buildTool === 'Yarn' || buildTool === 'pnpm') {
    ...
    else if (fnames.has('webpack.config.js') || fnames.has('webpack.config.ts'))buildTool = 'Webpack';
  }
  ...
  const analysis = {
    // Metadata
    projectName, description, version, author, license,
    keywords: pkgJson?.keywords || [],
    homepage: pkgJson?.homepage || '',
    repository: typeof pkgJson?.repository === 'string' ? pkgJson.repository : (pkgJson?.repository?.url || ''),

    // Language stats
    primaryLanguage, languages, languageStats, langColors: LANG_COLORS,
    ...
    configData: {
      pomData, gradleData, pkgJson, appProps, appYml,
      cargoData, goModData, pyprojectData,
      ...
      appName: appProps?.appName || appYml?.appName || pkgJson?.name || cargoData?.name || goModData?.moduleName || null,
      datasourceUrl: appProps?.datasourceUrl || appYml?.datasourceUrl || null,
      jpaDdlAuto: appProps?.jpaDdlAuto || appYml?.jpaDdlAuto || null,
    },

    // Flags
    hasTests, hasDocker, hasCI, hasEnvFile, hasGitignore,
    hasLicense, hasReadme, hasContributing,
    ...
    javaClasses, pythonEntities, jsModules,
  };
  ...
  if (analysis.backendFramework || analysis.metaFramework) detectedSignals.push(`Framework: ${analysis.metaFramework || analysis.backendFramework}`);
  ...
  if (analysis.features.filter(f => f.confidence >= 70).length) detectedSignals.push(`Entities: ${analysis.features.filter(f => f.confidence >= 70).length}`);
  if (analysis.architecture?.pattern) detectedSignals.push(`Architecture: ${analysis.architecture.pattern}`);
