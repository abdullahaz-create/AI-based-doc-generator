...
async function handleZipUpload(file, onFilesReady) {
  ...
  try {
    ...
    for (const [path, zipFile] of entries) {
      if (zipFile.dir) {
        files.push({ path, name: path.split('/').filter(Boolean).pop() || '', isDir: true, size: 0, content: null });
        continue;
      }
      ...
      const TEXT_NAMED_FILES = new Set([
        ...
        'cargo.toml', 'go.mod', 'go.sum', 'pipfile', 'poetry.lock',
        '.gitignore', '.dockerignore', '.prettierrc', '.eslintrc',
        '.editorconfig', '.babelrc', '.nvmrc', '.npmrc', '.yarnrc',
      ...
      const TEXT_EXTS = new Set([
        ...
        '.cs','.swift','.dart',
        // Scripting / Dynamic
        '.rb','.php','.lua','.r','.ex','.exs','.erl','.hrl',
        '.pl','.pm','.tcl','.awk','.fish','.zsh','.sh','.bash','.ps1',
        // Web
        '.vue','.svelte','.html','.htm','.css','.scss','.sass','.less','.styl',
        // Data / Config
        '.json','.yaml','.yml','.toml','.xml','.ini','.cfg','.conf',
        '.properties','.env','.env.example','.env.sample','.env.local',
        // Documentation
        '.md','.mdx','.rst','.txt','.adoc',
        // Query / Schema
        '.sql','.graphql','.gql','.prisma',
        // Build / Package
        '.lock','.example','.sample','.template','.tf','.hcl','.bicep',
        // Other
    ...
    setUploadProgress(95, 'Finalizing…', files.filter(f => !f.isDir).length, formatBytes(file.size), detectLangsQuick(files));
    await sleep(200);
...
function detectLangsQuick(files) {
  const counts = {};
  const EXT_LANG = { '.js':'JS', '.ts':'TS', '.py':'Python', '.java':'Java', '.go':'Go', '.rs':'Rust', '.rb':'Ruby', '.php':'PHP', '.vue':'Vue', '.svelte':'Svelte' };
  for (const f of files) {
    const lang = EXT_LANG[f.ext];
    if (lang) counts[lang] = (counts[lang] || 0) + 1;
  }
  const top = Object.entries(counts).sort((a,b) => b[1]-a[1]).slice(0,3).map(([l]) => l);
...
function validateProjectZip(files) {
  ...
  const SRC_EXTS = new Set([
    ...
    '.cs','.swift','.dart','.rb','.php',
    '.vue','.svelte','.ex','.exs','.erl','.hs','.lua',
  ]);
  ...
  if (!hasConfig && srcFiles.length < 3) {
    ...
    if (nonSrc === 0) {
      return { valid: false, warning: false, message: 'The ZIP appears to be empty or contains only folders.' };
    }
    return {
  ...
  if (hasConfig && srcFiles.length === 0) {
    return {
      ...
      message: 'Config file found but no source code detected. Documentation will be limited — add source files for richer output.',
    };
  }
...
async function handleGithubImport(url, onFilesReady) {
  ...
  try {
    ...
    if (!repoRes.ok) {
      throw new Error(repoRes.status === 404 ? 'Repository not found. Make sure it\'s public.' : `GitHub API error: ${repoRes.status}`);
    }
    ...
    const IMPORTANT_FILES = ['package.json', 'requirements.txt', 'pom.xml', 'build.gradle', 'composer.json', 'Gemfile', 'go.mod', 'Cargo.toml', '.env.example'];
    const rawBase = `https://raw.githubusercontent.com/${owner}/${repoName}/${branch}`;

    for (const fname of IMPORTANT_FILES) {
      const fileEntry = (treeData.tree || []).find(t => t.path === fname || t.path.endsWith('/' + fname));
      if (fileEntry) {
        try {
          const r = await fetch(`${rawBase}/${fileEntry.path}`);
          if (r.ok) {
            const content = await r.text();
            files.push({ path: fileEntry.path, name: fname, isDir: false, size: content.length, content, ext: fname.slice(fname.lastIndexOf('.')) });
    ...
    if (!files.find(f => f.name === 'package.json')) {
      files.unshift({ path: 'package.json', name: 'package.json', isDir: false, size: 500, content: JSON.stringify(pkgJson), ext: '.json' });
    }
...
function loadDemoProject(onFilesReady) {
  const pkg = {
    ...
    main:        'src/server.js',
    scripts:     { start: 'node src/server.js', dev: 'nodemon src/server.js', test: 'jest --coverage', build: 'tsc -p tsconfig.json' },
    dependencies: {
      express: '^4.18.2', mongoose: '^7.4.0', jsonwebtoken: '^9.0.1',
      bcryptjs: '^2.4.3', dotenv: '^16.3.1', cors: '^2.8.5',
      helmet: '^7.0.0', morgan: '^1.10.0', 'express-validator': '^7.0.1',
      multer: '^1.4.5-lts.1', nodemailer: '^6.9.4', winston: '^3.10.0',
    },
    devDependencies: {
      nodemon: '^3.0.1', jest: '^29.6.1', supertest: '^6.3.3',
      eslint: '^8.45.0', '@types/node': '^20.4.5',
    },
  ...
  const files = [
    { path: 'package.json', name: 'package.json', content: JSON.stringify(pkg, null, 2), size: 1200 },
    { path: 'package-lock.json', name: 'package-lock.json', content: '{}', size: 45000 },
    { path: 'src/server.js', name: 'server.js', content: `const express = require('express');\nconst mongoose = require('mongoose');\nconst cors = require('cors');\nrequire('dotenv').config();\nconst app = express();\napp.use(express.json());\napp.use('/api/auth', require('./routes/auth'));\napp.use('/api/tasks', require('./routes/tasks'));\napp.use('/api/users', require('./routes/users'));\napp.use('/api/projects', require('./routes/projects'));\napp.get('/health', (req, res) => res.json({ status: 'ok' }));\nmongoose.connect(process.env.MONGODB_URI).then(() => app.listen(process.env.PORT || 3000));`, size: 600 },
    { path: 'src/routes/auth.js', name: 'auth.js', content: `const router = require('express').Router();\nrouter.post('/login', loginController);\nrouter.post('/register', registerController);\nrouter.post('/logout', logoutController);\nrouter.get('/me', authenticate, getMeController);\nrouter.post('/refresh-token', refreshTokenController);\nmodule.exports = router;`, size: 280 },
    { path: 'src/routes/tasks.js', name: 'tasks.js', content: `const router = require('express').Router();\nrouter.get('/', authenticate, getTasks);\nrouter.post('/', authenticate, createTask);\nrouter.get('/:id', authenticate, getTask);\nrouter.put('/:id', authenticate, updateTask);\nrouter.delete('/:id', authenticate, deleteTask);\nrouter.patch('/:id/complete', authenticate, completeTask);\nmodule.exports = router;`, size: 320 },
    { path: 'src/routes/users.js', name: 'users.js', content: `const router = require('express').Router();\nrouter.get('/', authenticate, getUsers);\nrouter.get('/:id', authenticate, getUser);\nrouter.put('/:id', authenticate, updateUser);\nrouter.delete('/:id', authenticate, deleteUser);\nmodule.exports = router;`, size: 240 },
    { path: 'src/routes/projects.js', name: 'projects.js', content: `const router = require('express').Router();\nrouter.get('/', authenticate, getProjects);\nrouter.post('/', authenticate, createProject);\nrouter.get('/:id', authenticate, getProject);\nrouter.put('/:id', authenticate, updateProject);\nrouter.delete('/:id', authenticate, deleteProject);\nrouter.post('/:id/members', authenticate, addMember);\nmodule.exports = router;`, size: 370 },
    { path: 'src/models/Task.js', name: 'Task.js', content: `const mongoose = require('mongoose');\nconst TaskSchema = new mongoose.Schema({ title: { type: String, required: true }, description: String, status: { type: String, enum: ['todo','in-progress','done'], default: 'todo' }, assignee: mongoose.Types.ObjectId, project: mongoose.Types.ObjectId, dueDate: Date, priority: { type: String, enum: ['low','medium','high'], default: 'medium' } }, { timestamps: true });\nmodule.exports = mongoose.model('Task', TaskSchema);`, size: 520 },
    { path: 'src/models/User.js', name: 'User.js', content: `const mongoose = require('mongoose');\nconst UserSchema = new mongoose.Schema({ name: { type: String, required: true }, email: { type: String, required: true, unique: true }, password: String, role: { type: String, enum: ['user','admin'], default: 'user' }, avatar: String }, { timestamps: true });\nmodule.exports = mongoose.model('User', UserSchema);`, size: 420 },
    { path: 'src/models/Project.js', name: 'Project.js', content: `const mongoose = require('mongoose');\nconst ProjectSchema = new mongoose.Schema({ name: String, description: String, owner: mongoose.Types.ObjectId, members: [mongoose.Types.ObjectId], status: String }, { timestamps: true });\nmodule.exports = mongoose.model('Project', ProjectSchema);`, size: 360 },
    { path: 'src/middleware/auth.js', name: 'auth.js', content: `const jwt = require('jsonwebtoken');\nconst authenticate = (req, res, next) => { const token = req.headers.authorization?.split(' ')[1]; if (!token) return res.status(401).json({ message: 'Unauthorized' }); jwt.verify(token, process.env.JWT_SECRET, (err, user) => { if (err) return res.status(403).json({ message: 'Forbidden' }); req.user = user; next(); }); };\nmodule.exports = { authenticate };`, size: 480 },
    { path: 'src/config/database.js', name: 'database.js', content: `const mongoose = require('mongoose');\nmodule.exports = { connect: () => mongoose.connect(process.env.MONGODB_URI, { useNewUrlParser: true, useUnifiedTopology: true }) };`, size: 180 },
    { path: 'src/utils/logger.js', name: 'logger.js', content: `const winston = require('winston');\nmodule.exports = winston.createLogger({ level: 'info', format: winston.format.json(), transports: [new winston.transports.Console()] });`, size: 200 },
    { path: '.env.example', name: '.env.example', content: `PORT=3000\nMONGODB_URI=mongodb://localhost:27017/taskflow\nJWT_SECRET=your_super_secret_jwt_key_here\nJWT_EXPIRES_IN=7d\nEMAIL_HOST=smtp.gmail.com\nEMAIL_PORT=587\nEMAIL_USER=your@email.com\nEMAIL_PASS=your_app_password\nCORS_ORIGIN=http://localhost:3000\nNODE_ENV=development\nLOG_LEVEL=info`, size: 320 },
    { path: '.gitignore', name: '.gitignore', content: `node_modules/\n.env\ndist/\n*.log\ncoverage/\n.DS_Store\n.nyc_output/`, size: 90 },
    { path: 'tests/auth.test.js', name: 'auth.test.js', content: `const request = require('supertest');\ndescribe('Auth Routes', () => { it('should register a new user'); it('should login with valid credentials'); it('should reject invalid credentials'); it('should refresh token'); });`, size: 220 },
    { path: 'tests/tasks.test.js', name: 'tasks.test.js', content: `describe('Task Routes', () => { it('should return all tasks'); it('should create a task'); it('should update a task status'); it('should delete a task'); });`, size: 200 },
    { path: 'jest.config.js', name: 'jest.config.js', content: `module.exports = { testEnvironment: 'node', collectCoverageFrom: ['src/**/*.js'], coverageThreshold: { global: { branches: 70, functions: 80, lines: 80 } } };`, size: 180 },
    { path: '.eslintrc.json', name: '.eslintrc.json', content: `{"extends": "eslint:recommended", "env": {"node": true, "es2021": true}, "rules": {"no-unused-vars": "warn"}}`, size: 110 },
    { path: 'README.md', name: 'README.md', content: `# TaskFlow API\nA task management REST API.`, size: 40 },
    { path: 'vite.config.js', name: 'vite.config.js', content: `import { defineConfig } from 'vite';\nexport default defineConfig({});`, size: 60 },
  ].map(f => ({ ...f, ext: f.name.lastIndexOf('.') >= 0 ? f.name.slice(f.name.lastIndexOf('.')).toLowerCase() : '', isDir: false }));
