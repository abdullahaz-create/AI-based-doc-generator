...
async function _generateDocsWithFallback(analysis) {
  ...
  const results = await Promise.allSettled(
    _DOC_TYPES.map(docType =>
      window._aiClient.requestAIDoc(docType, analysisContext)
  ...
  if (aiSuccessCount === _DOC_TYPES.length) {
    if (generatingDetail) {
      generatingDetail.textContent =
        `${aiSuccessCount} documents generated by Gemini AI ✨`;
    }
    ...
    if (generatingDetail) {
      generatingDetail.textContent =
        `${aiSuccessCount} AI + ${aiFailureCount} template docs`;
    }
  } else {
...
function _serializeAnalysis(analysis) {
  return {
    ...
    folderStructure: analysis.folderStructure
      ? { name: analysis.folderStructure.name, children: (analysis.folderStructure.children || []).slice(0, 30) }
      : null,
  };
}
